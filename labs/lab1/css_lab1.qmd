...
In this lab, we'll explore how to use an API (Application program interface) to obtain FB monthly active user counts by geography, gender, and age. These data can be used to study topics such as migration or digital inequality. For example:
...
To query the Facebook Marketing API, we'll use the `rsocialwatcher` package. This R package helps us query the FB Marketing API.

The package requires credentials and a user-provided target population (e.g., women aged 15-49 in France). The package can then query the API and return a dataframe containing the number of Facebook monthly active users for your specified audience.
...
source("../../private/credentials_group3.R") 
