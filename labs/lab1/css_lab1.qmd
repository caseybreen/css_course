...
## All Facebook users in Great Britain between ages of 18 and 65 
fb_mau_users_france <- query_fb_marketing_api(
  location_unit_type = "countries",
  location_keys = "FR",
  age_min = 18, 
  age_max = 65,
...
fb_mau_users_france$estimate_mau_upper_bound

fb_mau_users_france %>% select(estimate_mau_upper_bound)
...
fb_mau_users_gb_40_50 <- query_fb_marketing_api(
  location_unit_type = "countries",
  location_keys = "GB",
  age_min = 40, 
  age_max = 50,
...
fb_mau_users_gb_30_40$estimate_mau_upper_bound - fb_mau_users_gb_40_50$estimate_mau_upper_bound
...
india_age_mau %>% count(gender_recode, gender)
```
...
2.  Make a bar plot of the Female MAU in each country. Make a bar plot of Male MAU in each country. Bonus: Make one plot with both male and female using different colors. (Note: you will need a long data file)
3.  In what settings would the FB gender gap not be a good predictor of internet gender gaps? What other predictors might be used in conjunction with the Facebook gender gap data to predict internet gender gaps?
...
all_countries_gender %>% 
  filter(gender == 1) %>% 
  ggplot(aes(x = location_keys, y = estimate_mau_upper_bound)) + 
  geom_col(position = position_dodge(1)) + 
  cowplot::theme_cowplot() + 
  scale_y_continuous(labels = scales::comma, limits = c(0, NA)) + 
  labs(x = "Country", y = "FB Users")
...
all_countries_gender %>% 
  ggplot(aes(x = location_keys, y = estimate_mau_upper_bound, fill = gender)) + 
  geom_col(position = position_dodge(1)) + 
  cowplot::theme_cowplot() + 
  theme(legend.position = "bottom") + 
  scale_y_continuous(labels = scales::comma, limits = c(0, NA))
