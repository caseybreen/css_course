...
To query the Facebook Marketing API (Application Program Interface), we'll use the `rsocialwatcher` package. This R package facilitates querying the FB Marketing API.

At a high-level, the package will allow you to give your credentials and some target population (e.g., women aged 15-49 in France). The package then queries the API and returns a dataframe containing the number of Facebook monthly active users for your specified audience.
