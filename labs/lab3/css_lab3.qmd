...
In this lab, we will explore different statistical methods for adjust non-probability samples to better estimate population-level statistics.

Non-probability sampling is increasingly common in data collection and, in some cases, the only feasible approach for reaching certain populations (hard-to-reach). For an overview of non-probability sampling in the social sciences, see:
...
Our second example will involve estimating mortality rates using a non-probability sample.

## Exercise 1

In this exercise, we'll calculate basic post-stratification weights based on a simulated convenience sample. A *survey weight* is a value assigned to each respondent in a survey to adjust for unequal probabilities of selection, nonresponse, and other biases, ensuring that the sample represents the target population accurately.
...
This (hypothetical) convenience sample was taken at a coffee shop. Respondents were asked about their age, income, and daily coffee consumption in cups. We've also obtained auxiliary city-level data on people aged 18+ that we can use to reweight on age and income.
...
We'll also read in our auxiliary data from the city on the true proportion of people in each group.
...
Post-stratification weighting is a statistical re-weighting approach used to correct for discrepancies between a sample and the true population distribution. Specifically, the population is split into mutually extensive groups ("cells") based on characteristics (e.g., age, gender, and race). Each cell's survey responses are then weighted according to known population distributions:
...
When implementing post-stratification in practice, it's important that your sample collects the same information as is available in the auxiliary population data (i.e., design your survey instrument in such a way to collect all the data you need for weighting!).

The code below will generate post-stratification weights on income and age group.

```{r}
## calculate sample porportion 
...
## 1.3 
sample_data_weighted %>% 
  summarize(unweighted = mean(coffee_consumption * weight),
            weighted = mean(coffee_consumption))
...
***Estimating mortality rates with a non-probability sample***

In Exercise 2, we are apply a new method for estimating mortality rates in humanitarian emergencies. This method combines the network survival method with non-probability sampling—the type of sample we might be able to collect in a humanitarian emergency (hurricanes, civil wars, etc.). In such settings, researchers often face significant barriers to collecting truly random samples, making non-probability approaches necessary.

The data collected for this exercise come from the Tanganyika Province of the Democratic Republic of the Congo (DRC). A quota sample drawn from key local hubs, including taxi stations, ports, markets, and hospitals.

In this exercise, we will assess the impact of different weighting strategies on our mortality estimates. Specifically, we will apply two different weighting techniques: post-stratification and inverse-probability weighting.

Background: Our quota sample was likely to over-represent more affluent people, who we anticipate would have lower mortality in their social network.
...
We will estimate a mortality rate as expressed in deaths per 10,000 person days. This is the conventional way to express deaths for humanitarian emergencies; it contrasts with the conventional demographic definition of deaths per 1,000 person-years.

To estimate the crude death rate (our estimand of interest), we use the following estimator:
...
Let’s focus on kin network as an example. Calculating a death rate requires two key components, the number of deaths and the measure of exposure.

-   Counting deaths: This is simply the total number of kin deaths reported by everyone in the sample.
...
The mortality rate is then obtained by dividing the total number of deaths by the total exposure and multiplying by 10,000 to express the rate per day. We will compute this separately for the kin and network sample to produce two distinct estimates of the death rate.

We'll estimate the rates without applying weights:
...
```{r}
## read in post-stratification weights 
...
poststrat_worldpop <- poststrat_weighting_targets %>%
...
poststrat_weights <- poststrat_sample %>%
  inner_join(poststrat_worldpop, by = c("age_class", "gender")) %>%
...
The distribution of weights looks reasonable: there are no huge weights. We're read to calculate the weighted estimates. Note that we need to explicitly tell our function what weight column to use.
...
Our post-stratification weights increased our estimates of the crude mortality rate, but only slightly. However, this re-weighting may not fully account for all factors. Recall, we didn't For example, wealthier individuals are more likely to be connected to other wealthy individuals, who we would expect to have lower death rates.

To better adjust for this, we need additional characteristics. The challenge with post-stratification is the "curse of dimensionality" — as the number of stratification variables increases, cells become small, sparse, and produce extremely large weights, making inference difficult.
...
We're worried that our sample taken at major hubs is overrepresenting wealthier individuals. To account for this, we'll use these four covariates that proxy for wealth: `manufactured_material_house`, `bed`, `radio` and `modern_fuel_type`. 

**2.1** Recalculate the weights using logistic regression model with the following additional predictors: `manufactured_material_house` `bed`, `radio` and `modern_fuel_type`. These are classic predictors of wealth. Are these predictors statistically significant in our logistic regression model?

**2.2** Re-estimate the crude death rate using the new weights. How does your crude death rate change? Make a visualization of the unweighte estimate, post-stratification estimate, and the overall estimate.

**2.3** Why do we think that our crude rates are changing when our weights account for socioeconomic status. What does this tell us about the relative mortality risk of people people with low SES?
...
-   Calculate separate crude death rates by health zone using no weights, post-stratification weights, and inverse probability weights (with SES variables). What health zone has the highest crude death rate?
