...
Our second example will involve estimating mortality rates using a non-probability sample. We will use two different methods for.

## Exercise 1

In this exercise, we'll calculate basic post-stratification weights based on a simulated convenience sample. This convenience sample was taken at a coffee shop. Respondents were asked about their age, income, and daily coffee consumption in cups. We've also obtained auxiliary city-level data on people aged 18+ that we can use to reweight on age and income.
...
First, we'll load our simulated data and caluclate the 
...
coffee_shop_sample_proportions <- coffee_shop_sample %>% 
...
1.  Make a histogram of our weights using `ggplot` and `geom_histogram()`
...
-   Measuring exposure: We calculate this by summing the total number of kin for each person, multiplied by their number of days of exposure. The mortality rate is then obtained by dividing the total number of deaths by the total exposure and multiplying by 10,000 to express the rate per day.
...
As we're going to do this multiple times, let's write a function. The functions will have a few different arguments, including:
...
The function will output the numerator, denominator. If no weights are supplied, it will generate unweighted estimates.
...
Now let's just using the function to confrim what we calculated above: 
...
Our estimates from the function align exactly with our previous estimates. 
...
weighting_targets <- read_csv("../../data/weighting_targets_poststrat.csv")
...
Now let's calculate the weighted estimates. Note that we need to explicitly tell our function what set of weights to us. 
...
Most weights are fairly small, but some are substantial, even greater than 5. We can now calculate our crude death rate. 
...
Our estimates are very similar regardless of whether we use post-stratification weights or inverse probability weigts. However, we have more covariates we can use for inverse probability weights that capture socioeconomic status (SES). 

Exercise 2:

We're worried that our sample taken at major hubs is overrepresenting wealthier individuals. To account for this, we'll use these four covariates 

**2.1** Recalculate the weights using logistic regression model that has the following additional predictors: `manufactured_material_house` `bed`, `radio` and `modern_fuel_type`. These are classic predictors of wealth. Are these predictors statistically significant in our logistic regression model?
