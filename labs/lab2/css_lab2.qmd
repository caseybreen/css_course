...
lm_model_performance <- sim_data_test_w_predictions %>% 
  summarize(rmse = rmse(actual = stress, predicted = predicted_stress),
            mae = mae(actual = stress, predicted = predicted_stress),
            r_squared = r_squared(actual = stress, predicted_stress))
...
```{r}
sim_data_test_rf_predictions %>% 
  summarize(rmse = rmse(actual = stress, predicted = predicted_stress_rf),
            mae = mae(actual = stress, predicted = predicted_stress_rf),
...
ba <- function(actual, predicted){
  mean(predicted - actual)
}


sim_data_test_rf_predictions %>% 
  summarize(bias = ba(actual = stress, predicted = predicted_stress_rf))
```
...
```{r}
## create folds 
folds_count_loco <- group_vfold_cv(internet_df, group = country)
folds_loco 
...
## get error metrics  
error_metrics_loco <- collect_metrics(rf_res) %>% 
  mutate(cv_method = "leave-one-country-out")
...
```{r}
cowplot::plot_grid(loco_calibration_plot, fold_10_plot)
```
